...
function getShortMonth(month, locale) {
    ...
    const result = new Intl.DateTimeFormat(locale, {month: "short"}).format(new Date(2000, month - 1));
...
function formatDateOrRange(dateOrRange, locale) {
    ...
    if (!dateOrRange.dateRange)
        return "";
    const from = dateOrRange.dateRange.from && formatDate(dateOrRange.dateRange.from);
